...
Isolation X-Y (1/$s_{xy}$) was measured to be under 50dB (Pin/Pout$<10^{-5}=3.162\times10^{-3}$ voltage isolation), sensitivity to pitch ($s_p$) was measured to be $3.2\mu$m/mrad, and sensitivity to yaw ($s_y$) is 2.9$\mu$m/mrad estimated in similar way to $s_p$. Precision on phase rotation ($\Delta\phi$) is still to be determined. Expresing the previous equation in nm and $\mu$rad, we obtain
\begin{equation}
 S_j=y_j+x_j(1.880\times10^{-3}+10^{-6}\theta_{r0})+|\Delta\phi|(3.2y'_j+5.453\times10^{-3}x'_j)
\end{equation}
